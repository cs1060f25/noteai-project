...
This document defines coding standards, best practices, and architectural patterns for the backend services of the AI Lecture Highlight Extractor project.
...
5. [Celery & Task Queue Patterns](#celery--task-queue-patterns)
6. [AWS S3 & Cloud Services](#aws-s3--cloud-services)
...
use **uv** as the modern python package and project manager. uv is significantly faster than pip and provides better dependency resolution.
...
```toml
[project]
name = "lecture-highlight-extractor"
version = "0.1.0"
...
[tool.ruff]
# see ruff configuration section below
line-length = 100
target-version = "py39"

[tool.black]
# see black configuration section below
line-length = 100
target-version = ['py39']
...
**ruff** is an extremely fast python linter written in rust. it replaces flake8, isort, and several other tools.
...
[tool.ruff.isort]
known-first-party = ["app", "agents", "pipeline"]
...
**black** is the uncompromising python code formatter. it enforces consistent style across the entire codebase.
...
services:
  ...
  api:
    ...
    ports:
      - "8000:8000"
    environment:
      - DATABASE_URL=postgresql://postgres:password@db:5432/lecture_db
  ...
  worker:
    build:
      context: .
      dockerfile: Dockerfile
    command: uv run celery -A app.core.celery_app worker --loglevel=info -Q default,high_memory,high_cpu
    environment:
      - DATABASE_URL=postgresql://postgres:password@db:5432/lecture_db
      - REDIS_URL=redis://redis:6379/0
      - CELERY_BROKER_URL=redis://redis:6379/0
...
docker-compose exec api uv run black .
...
```makefile
.PHONY: install lint format test run docker-up docker-down clean
...
from app.core.dependencies import get_s3_service, get_db_service
from app.models.upload import UploadRequest, UploadResponse, PresignedUrlResponse
from app.services.s3_service import S3Service
from app.services.db_service import DatabaseService
...
async def generate_upload_url(
    ...
    directly to s3 without proxying through the backend server.

    args:
        request: upload request containing filename and content type
        s3_service: injected s3 service dependency
        settings: injected application settings

    returns:
        presigned url with expiration time and upload fields

    raises:
        HTTPException: if file size exceeds limit or invalid file type
    """
    # validate file size
    if request.file_size_mb > settings.max_video_size_mb:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail=f"file size exceeds maximum of {settings.max_video_size_mb}mb"
        )

    # validate file type
    if not _is_valid_video_format(request.filename):
        raise HTTPException(
    ...
    s3_key = f"uploads/{request.user_id}/{request.job_id}/{request.filename}"
    ...
    try:
        ...
        # log error and return generic message to client
        # avoid exposing internal aws details
...
async def complete_upload(
    ...
    """
    finalize upload and trigger processing pipeline.

    called by frontend after successful s3 upload to initiate
    lecture processing through celery task queue.

    args:
        request: completion request with s3_key and job_id
    ...
    # don't block response waiting for celery
    background_tasks.add_task(
        trigger_lecture_pipeline,
        job_id=job_id,
        s3_key=s3_key
    )
...
def _is_valid_video_format(filename: str) -> bool:
...
async def generic_exception_handler(
    ...
    logs full exception details while returning safe error
    message to client without exposing internals.
    """
    logger.error(
...
app = FastAPI(title="AI Lecture Highlight Extractor")
