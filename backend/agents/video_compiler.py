...
class VideoCompiler:
    ...
    def _process_segment(
        ...
        """Process a single content segment into a clip, safely handling
        any segment duration relative to the actual video length.

        Returns:
            Clip metadata dictionary, or None if the segment is invalid.
        """
        clip_id = str(uuid.uuid4())
        logger.info(
            "Processing segment",
            extra={"job_id": job_id, "segment_id": segment.segment_id, "clip_id": clip_id},
        )

        # --- validate and clamp segment times ---
        video_duration = float(video_info.get("duration", 0))
        start_time = max(0.0, float(segment.start_time))
        end_time = min(float(segment.end_time), video_duration)

        # skip invalid or zero-length segments
        if end_time <= start_time or start_time >= video_duration:
            logger.warning(
                f"Skipping segment {segment.segment_id}: "
                f"invalid time range ({start_time:.2f}-{end_time:.2f}s) "
                f"for video length {video_duration:.2f}s"
            )
            return None

        # --- extract the valid portion of the segment ---
        ...
        # --- re-encode or pass-through if resolution matches ---
        ...
        # --- add metadata ---
        metadata = {
            "title": segment.topic or f"Segment {clip_order + 1}",
        ...
        segment_transcripts = self._get_segment_transcripts(transcripts, start_time, end_time)
