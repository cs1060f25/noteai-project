...
import { AlertCircle, ArrowLeft, Download, Layout, Loader2, Play, Timer } from 'lucide-react';
...
export const AgentOutputsDetailView = ({ jobId }: { jobId: string }) => {
  ...
  const [playerSize, setPlayerSize] = useState<'compact' | 'medium' | 'large'>(() => {
    return (
      (localStorage.getItem('noteai-player-size') as 'compact' | 'medium' | 'large') || 'medium'
    );
  });
