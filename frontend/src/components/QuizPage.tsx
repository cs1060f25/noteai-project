...
export function QuizPage({ onBack, lectureTitle, questions }: QuizPageProps) {
    ...
    const getScoreMessage = (score: number) => {
        if (score >= 90) return { title: '🎉 Outstanding!', message: 'You mastered this material!', color: 'text-green-500' };
        if (score >= 70) return { title: '👏 Great Job!', message: 'You have a solid understanding!', color: 'text-blue-500' };
        if (score >= 50) return { title: '👍 Good Effort!', message: 'Keep studying to improve!', color: 'text-yellow-500' };
        return { title: '📚 Keep Learning!', message: 'Review the material and try again!', color: 'text-orange-500' };
    };
    ...
    if (isQuizComplete) {
        ...
        const avgTime = answers.length > 0 ? Math.round(answers.reduce((acc, a) => acc + a.timeSpent, 0) / answers.length) : 0;
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            ...
                            >
                                ...
                                <div className="text-2xl mb-1">{correctCount}/{quizQuestions.length}</div>
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-muted-foreground">Quiz Master - Scored 90% or higher</p>
                                    </div>
                        ...
                        <div className="space-y-3">
                            {quizQuestions.map((q, index) => {
                                ...
                                return (
                                    <div
                                        key={q.id}
                                        className={`p-4 rounded-xl border transition-colors ${answer?.isCorrect
                            ...
                            {currentQuestion.options.map((option, index) => {
                                ...
                                return (
                                    <motion.button
                                        ...
                                        className={`w-full p-4 rounded-2xl border-2 transition-all text-left relative overflow-hidden ${showCorrect
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div
                                                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${showCorrect
                                            ...
                                            >
                                                ...
                                                {!showExplanation && (
                                                    <span className={isSelected ? 'text-primary-foreground text-sm' : 'text-muted-foreground text-sm'}>
                                                        {String.fromCharCode(65 + index)}
                                                    </span>
                                                )}
                                            </div>
                                            <span className={showExplanation && (showCorrect || showIncorrect) ? '' : ''}>{option}</span>
                                        </div>
                                    </motion.button>
                                );
                            })}
